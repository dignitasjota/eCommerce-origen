...
| Backlog premium | 🟡 12/14 — resto fuera de alcance incremental | Veri*Factu (base local, sin certificado) completado 2026-09-15 (junto a los 11 anteriores); los 2 restantes son cambios de modelo de negocio — ver bloque dedicado |
...
- [x] **Multi-warehouse / drop shipping** — `Warehouse` (código, prioridad de asignación, flag `is_dropshipping`) + `WarehouseStock` (desglose real de stock por variante×almacén). `ProductVariant.stock` pasa a ser un total cacheado (mismo patrón que `User.loyalty_points`) para que todo el código de lectura pre-existente (storefront, countdown de stock, selección de variante) siga funcionando sin cambios. El checkout asigna almacén por prioridad ascendente con `updateMany`+guardia atómica (`src/lib/warehouse.ts`); un almacén de **drop shipping** tiene capacidad virtual — nunca bloquea una venta ni descuenta contador local. Las 4 rutas de restitución (webhook Stripe cancelado/reembolsado, cron `cleanup-pending-orders`, RMA `markReturnReceived`) comparten un único helper `restockToWarehouse` que repone al almacén de origen (`OrderItem.warehouse_id`), con fallback al almacén por defecto en pedidos anteriores a esta funcionalidad. Nuevo CRUD en `/admin/warehouses` + matriz "Stock por almacén" en la ficha de producto (sustituye el campo plano de stock, ahora sólo lectura del total). Bootstrap (`seed-bootstrap.mjs`) crea un almacén por defecto y migra el stock de variantes existentes de forma idempotente. **Bug real encontrado y corregido en la validación:** el checkout tenía un guard temprano (`variant.stock < quantity`) que rechazaba con 400 pedidos que sí podían cubrirse vía un almacén de drop shipping activo, porque el total cacheado excluye a propósito la capacidad virtual — eliminado; la única fuente de verdad es la asignación dentro de la transacción. Validado con Playwright (alta de almacenes → edición de stock por almacén vía matriz → checkout consume el almacén de menor prioridad con stock suficiente → un segundo checkout sin stock físico recae en drop shipping sin bloquear la venta → desactivar drop shipping sin stock físico devuelve 409 → webhook de cancelación restituye al almacén de origen → el detalle de pedido en el admin muestra el almacén asignado) + tsc/eslint/build limpios + suite e2e completa (19 passed/1 skipped/0 failed). ✅ 2026-09-14
- **Marketplace multi-vendor**.
- **Multi-tienda** (varios `SiteSettings` namespaces).
- [x] **Veri*Factu (AEAT) — base local de cumplimiento** — `InvoiceRecord` + `InvoiceChainState`: cada factura genera, dentro de la misma transacción que asigna `invoice_number` (`claimInvoiceNumber`), un registro de facturación con huella SHA-256 encadenada a la anterior (mismo espíritu que exige el Reglamento de facturación), QR normativo y desglose base/cuota de IVA (tipo configurable en Ajustes → Pagos, `invoice_vat_rate`). El bloqueo de concurrencia usa `SELECT ... FOR UPDATE` sobre un puntero singleton (`InvoiceChainState`) — a diferencia de `InvoiceCounter`, aquí el valor nuevo depende del contenido del anterior, no es un simple incremento atómico. Nuevo `/admin/invoicing` (listado cursor-paginado, botón "Verificar integridad de la cadena" que recalcula todas las huellas y detecta alteraciones retroactivas, export CSV). El PDF de factura ahora imprime el QR + huella. **No es una integración certificada:** sin certificado digital real no hay firma electrónica, y no hay envío a la AEAT en tiempo real — el `submission_status` de cada registro queda en `PENDING`, listo para conectar un proveedor homologado (Sage, Holded, B2Brouter) o el webservice oficial cuando exista certificado; avisos explícitos en el código y en la UI del admin. **Bug real pre-existente encontrado y corregido en la validación** (sin relación con Veri*Factu en origen): la generación de PDF de facturas llevaba rota desde que existe — `pdfkit` resuelve sus fuentes (.afm) con una ruta relativa a su propio código (`__dirname`), y Next.js la reescribía al empaquetarla a una ruta virtual del bundle (`/ROOT/...`) inexistente en disco, tanto en `next dev` como en `next start`/producción — nadie había probado el endpoint con una petición HTTP real hasta esta validación. Corregido con `serverExternalPackages: ['pdfkit']` en `next.config.ts`. De paso: un bug de precedencia de operadores en la resolución del nombre del cliente en el PDF, y `Order.tax` (antes hardcodeado a `0`) ahora refleja el IVA real. Validado con Playwright de extremo a extremo (8 pasos: configuración → checkout con IVA correcto → primer registro con `previous_hash` nulo → segundo registro encadenado al primero → visor admin → verificación de cadena OK → **detección de manipulación** alterando un registro a mano → PDF con QR → CSV) + tsc/eslint/build limpios + suite e2e completa (19 passed/1 skipped/0 failed). ✅ 2026-09-15
