...
Documento de referencia del proyecto. Estado actualizado al **2026-09-15**, tras los Sprints 1–5, tareas continuas, dockerización, operativa, compliance, escala UI, pre-producción, CI/CD, drag&drop de imágenes, editor WYSIWYG (Tiptap), una **auditoría de seguridad en profundidad con corrección + validación real de todos los hallazgos** (ver §9.9) y **12 de los 14 ítems del backlog premium** (reseñas con foto, stock countdown, wishlist compartible, comparador, WhatsApp, RBAC fino + auditoría completa, PWA, fidelización, matriz combinatoria de variantes, A/B testing del hero, multi-warehouse/drop shipping, Veri*Factu base local — detalle completo en [`ROADMAP.md`](./ROADMAP.md), no duplicado aquí). Roadmap ejecutivo en [`ROADMAP.md`](./ROADMAP.md).
...
- **`ProductVariant`** — SKU UNIQUE, precio override (null = hereda), `stock`, `is_active`. Pivot con `VariantOption` vía `ProductVariantOption`. **Stock real por variante** desde Sprint 5 (CRUD plano en admin). Desde 2026-09-14 (multi-warehouse), `stock` es un **total cacheado** (mismo patrón que `User.loyalty_points`) — la fuente de verdad real del desglose es `WarehouseStock`; sólo se escribe a través de `src/lib/warehouse.ts`.
- **`Category`** — `slug` UNIQUE, jerarquía con `parent_id`, traducciones independientes, `image`, `sort_order`, `is_active`.
- **`Order`** — `order_number` UNIQUE (`ORD-YYYYMM-XXXXXXXX`, no enumerable), `status` (enum `OrderStatus` que incluye `PENDING_PAYMENT`), `payment_status`, `fulfillment_status` (enum `FulfillmentStatus` separado), `payment_method`, `payment_id`, `payment_intent_id` UNIQUE (Stripe), `invoice_number` UNIQUE, `invoice_url`, `subtotal/shipping_cost/discount/tax/total` (`tax` desde 2026-09-15 refleja el desglose informativo de IVA calculado en checkout — antes hardcodeado a `0`), `coupon_id`, `tracking_number`, `locale`, `delivered_at` (nullable, 2026-09-10 — fijado sólo al entrar en `DELIVERED`, base de la ventana de devolución; ver §9.9). Relaciones con `User`, `Address` (shipping/billing), `ShippingMethod`, `OrderItem`, `Coupon`, `InvoiceRecord`. Índice en `created_at` (usado por la paginación cursor de `/admin/orders` y por el cron `cleanup-pending-orders`).
...
- **`User`** — `email` UNIQUE, `password_hash`, `name`, `phone`, `role` (`ADMIN`/`ORDER_MANAGER`/`CUSTOMER`), `is_active`, `last_login_at`, `image`, `permissions` (JSON nullable, RBAC fino — ver §9.9 backlog premium/`lib/permissions.ts`), `loyalty_points` (Int, default 0), `wishlist_share_token` (String? UNIQUE, 256 bits hex). Relaciones con `Address`, `Order`, `CartItem`, `Review`, `WishlistItem`, `Account`, `Session`, `LoyaltyTransaction`.
...
- **`StockMovement`** — `(variant_id, quantity ±, reason, reference_id, warehouse_id, note, user_id)`. Enum `StockMovementReason`: `PURCHASE | REFUND | ADJUSTMENT | RESTOCK | RESERVATION_RELEASE`. `warehouse_id` (2026-09-14) es un campo suelto sin relación FK — a propósito, mismo criterio que `variant_id`: el registro de auditoría debe sobrevivir aunque el almacén se borre. Cableado — ver §5.8.

### Entidades del backlog premium (2026-09-10 → 2026-09-15, ver `ROADMAP.md`)

- **`LoyaltyTransaction`** — ledger de puntos de fidelización, mismo patrón que `StockMovement`/`AuditLog`. `(user_id, order_id?, points ±, type, note, created_at)`. Enum `type`: `EARN | REDEEM | REVERSAL`. Cableado en `src/lib/loyalty.ts`.
- **`AbTestEvent`** — evento anónimo (sin relación a `User`) del test A/B del hero: `(test_key, variant, event_type, created_at)`. `event_type`: `impression | click`. Ver `src/lib/ab-testing.ts`.
- **`Warehouse`** (2026-09-14) — almacén físico o proveedor de drop shipping: `name`, `code` UNIQUE, dirección, `is_active`, `is_dropshipping` (capacidad virtual — nunca bloquea una venta ni descuenta stock local), `priority` (orden de asignación en checkout, menor = primero). Ver `src/lib/warehouse.ts`.
- **`WarehouseStock`** (2026-09-14) — desglose real de stock por `(variant_id, warehouse_id)` UNIQUE compuesto. Fuente de verdad del inventario; `ProductVariant.stock` es su suma cacheada.
- **`InvoiceRecord`** (2026-09-15) — registro de facturación encadenado (base local Veri*Factu, NO certificada — ver §5.9 y §9.9). `order_id` UNIQUE, `invoice_number` UNIQUE, `hash` UNIQUE (SHA-256), `previous_hash` (huella del registro anterior, `null` sólo en el primero), `qr_payload`, desglose de IVA, `submission_status` (hoy siempre `PENDING`). Ver `src/lib/verifactu.ts`.
- **`InvoiceChainState`** (2026-09-15) — puntero singleton (`id: 'default'`) al último `hash` de la cadena de `InvoiceRecord`. Se bloquea con `SELECT ... FOR UPDATE` para serializar la asignación de `previous_hash` bajo concurrencia.
...
2. **Rehidrata catálogo desde DB**: por cada item lee `product` + `variant` con `is_active=true`, **ignora `name`/`price` del cliente**. Precio definitivo viene de variant ?? product. Desde 2026-09-14 **no** hay guard temprano de `variant.stock < quantity` antes de la transacción — ver nota multi-warehouse abajo.
...
4. **Desglose informativo de IVA** (2026-09-15): `splitVatFromTotal(total, vatRate)` (`src/lib/verifactu.ts`) calcula base/cuota a partir del `total` YA con IVA incluido (precios con IVA incluido, práctica habitual B2C) — no se suma nada, es sólo el desglose que se persiste en `Order.tax` (antes hardcodeado a `0`) y que reutiliza el registro de facturación (§5.9).
5. **Transacción atómica** (`prisma.$transaction`):
   - Crear `Address` permanente.
   - Genera `orderId` con `randomUUID()` ANTES de crear la orden, para poder usarlo como `reference_id` de los `StockMovement` de asignación (ver siguiente punto) y pasarlo explícito a `tx.order.create({ data: { id: orderId, ... } })`.
   - **Asignación de almacén + decremento atómico** (multi-warehouse, 2026-09-14): `allocateAndDecrementStock(tx, {...})` (`src/lib/warehouse.ts`) recorre los almacenes activos por `priority` ascendente; si es de drop shipping (`is_dropshipping`) se asigna sin decrementar ningún contador local (capacidad virtual); si no, `updateMany({ where: { variant_id, warehouse_id, stock: { gte: q } }, ... })` — si ningún almacén cubre la cantidad, lanza `'Stock insuficiente'` ⇒ rollback (mismo mensaje, ahora HTTP 409 para todos los casos — el guard temprano en 400 que existía antes se eliminó porque rechazaba pedidos que un almacén de drop shipping sí podía cubrir). El `warehouse_id` resuelto se persiste en cada `OrderItem`.
   - Crear `Order` con `status=PENDING_PAYMENT` (Stripe) o `PENDING` (COD/TRANSFER), `coupon_id`, `tax` (desglose de IVA) y `order_items` (incluye `warehouse_id` por ítem).
   - `consumeCoupon(tx, coupon)` con `updateMany({ where: { id, used_count: { lt: max_uses } } })` — count !== 1 ⇒ rollback.
6. **Si Stripe**: crea Checkout Session con `metadata.order_id`, persiste `payment_intent_id` y `payment_id`. Devuelve `{ checkoutUrl }`.
7. **Si COD/TRANSFER**: envía email confirmación inmediato y devuelve `{ orderId }`.
...
4. **Handlers** (reescritos 2026-09-10, ver §9.9):
   - `checkout.session.completed`, `async_payment_succeeded` → orden `CONFIRMED + PAID` + `ensureInvoiceNumber()` (que desde 2026-09-15 también crea el `InvoiceRecord` encadenado, ver §5.9) + email confirmación.
   - `async_payment_failed`, `expired`, **`payment_intent.canceled`** → `revertOrderReservation()`: restituye stock **al almacén de origen** (`restockToWarehouse(tx, { warehouse_id: item.warehouse_id, ... })`, `src/lib/warehouse.ts` — fallback al almacén por defecto si `warehouse_id` es `null`, caso de pedidos anteriores a multi-warehouse), revierte `coupon.used_count` (guard `gt: 0`) y marca `CANCELLED + FAILED`, todo en una transacción con guard atómico (`updateMany` + `count===1`) — mismo patrón que el cron `cleanup-pending-orders`. Antes dejaba la orden en `CANCELLED` sin revertir nada, fuera del alcance de ese cron (que sólo mira `PENDING_PAYMENT`) ⇒ fuga permanente de stock y cupón en todo pago Stripe fallido/expirado.
   - `charge.refunded` → `handleChargeRefunded()`: distingue reembolso parcial de total comparando `amount_refunded` vs `amount`; consulta `stripe.refunds.list()` para detectar si el reembolso ya lo gestionó el flujo RMA (`refundReturn`, que restituye stock por ítem) y evitar duplicar la restitución; un reembolso total fuera del flujo RMA sigue el comportamiento legacy (restituye todo el stock vía `restockToWarehouse`, marca `REFUNDED`, revierte cupón); un parcial sin RMA no toca stock ni estado — sólo deja `AuditLog(order.partial_refund_unmanaged)` para revisión manual.
...
- Restituye stock al almacén de origen con `restockToWarehouse()` (`src/lib/warehouse.ts`) y registra `StockMovement RESERVATION_RELEASE`.
...
`src/lib/stock.ts` expone `recordStockMovement({ variant_id, quantity ±, reason, reference_id, warehouse_id, note, user_id }, tx?)`:
- Acepta opcionalmente un `tx` (cliente de transacción Prisma) → el log entra y sale con la transacción.
- Best-effort en errores; el inventario "real" vive en `WarehouseStock` desde 2026-09-14 (`ProductVariant.stock` es el total cacheado), este log es para auditoría/reportes.
- Desde 2026-09-14, todos los cambios de stock (checkout, restitución, ajustes de admin) pasan por `src/lib/warehouse.ts` (`allocateAndDecrementStock` / `restockToWarehouse` / `setWarehouseStock`), que llama a `recordStockMovement` internamente con el `warehouse_id` correcto — no llamar a `recordStockMovement` directamente para cambios de stock reales, sólo para casos sin desglose por almacén.
...
| `RESTOCK` | Editor de variantes o matriz "Stock por almacén" (admin) sube stock manualmente (positivo). |
| `ADJUSTMENT` | Editor de variantes o matriz "Stock por almacén" (admin) baja stock manualmente (negativo). |

### 5.9. Facturas y registro de facturación (Veri*Factu, base local)
...
- **`claimInvoiceNumber` (dentro de la transacción) también crea el `InvoiceRecord`** (2026-09-15, `createInvoiceRecord` en `src/lib/verifactu.ts`) — un número de factura nunca existe sin su registro correspondiente en la cadena. Requiere `invoice_seller_tax_id` configurado en `SiteSettings`; si falta, se omite (mejor no crear un registro incompleto) y queda trazado en logs.
- **Encadenamiento**: cada `InvoiceRecord.hash` (SHA-256) incluye la huella del anterior (`previous_hash`). La asignación se serializa bajo concurrencia con `SELECT ... FOR UPDATE` sobre el puntero singleton `InvoiceChainState` (a diferencia de `InvoiceCounter`, aquí el valor nuevo depende del contenido del anterior — un UPSERT atómico simple no basta). `verifyInvoiceChain()` recorre toda la cadena y recalcula cada huella para detectar alteraciones retroactivas — botón "Verificar integridad de la cadena" en `/admin/invoicing`.
- `src/lib/invoice-pdf.ts` — `buildInvoicePdf(data)` construye el PDF con **pdfkit** (A4, fuentes built-in, sin logos). Cabecera, vendedor/comprador, tabla items, totales, pie. Desde 2026-09-15 imprime también el QR (`qrcode` npm, PNG embebido) y la huella del `InvoiceRecord` si existe. **`pdfkit` debe estar en `serverExternalPackages` de `next.config.ts`** — ver §9.10, si no el `new PDFDocument()` revienta con ENOENT en producción.
- `GET /api/admin/orders/[id]/invoice.pdf` — autorización: admin/order_manager o **dueño de la orden**. Para usuarios no-admin requiere `payment_status === 'PAID'` (sin sentido fiscal antes). Datos del vendedor desde `SiteSettings`: `invoice_seller_tax_id`, `invoice_seller_address`, `invoice_seller_email`, `invoice_vat_rate` (default 21%, usado sólo para desglosar base/cuota, nunca para calcular el total a cobrar).
- `/admin/invoicing` — visor cursor-paginado del registro de facturación + botón de verificación de cadena + export CSV (`GET /api/admin/invoicing/export`, estructura propia, NO el XML oficial del SII/Veri*Factu).

> ⚠️ **Aviso fiscal:** la numeración aleatoria descrita arriba **fue sustituida** en el bloque Compliance (2026-05-02) por `InvoiceCounter` con secuencia atómica `<SERIE>-YYYY-NNNNNNN`. Ver §9.6. El registro de facturación encadenado (2026-09-15) **NO es una integración Veri*Factu certificada**: sin certificado digital real no hay firma electrónica, y no hay envío a la AEAT en tiempo real (`submission_status` queda en `PENDING`). Ver §9.10.
...
| Backlog premium | 2026-09-10 → 2026-09-15 | 12/14 ítems, cada uno validado individualmente contra Node 22 + MariaDB Docker + build + suite e2e | Reseñas con foto + verificación (y moderación admin nueva, antes inexistente) · stock countdown/urgencia · wishlist compartible · comparador de productos · WhatsApp Business · RBAC fino (`User.permissions`) + auditoría completa · PWA (manifest + service worker) · programa de fidelización (`LoyaltyTransaction`) · matriz combinatoria de variantes (arregló selección de variantes rota desde el origen — `ProductVariantOption` nunca se escribía) · A/B testing del hero (`AbTestEvent`) · multi-warehouse/drop shipping (`Warehouse`+`WarehouseStock`) · Veri*Factu base local (`InvoiceRecord` encadenado). Los 2 restantes (marketplace multi-vendor, multi-tienda) son cambios de modelo de negocio — detalle completo en [`ROADMAP.md`](./ROADMAP.md), no duplicado aquí. Ver §9.10. |
...
### 9.10. Multi-warehouse y Veri*Factu (2026-09-14 → 2026-09-15)

Últimos dos ítems de alcance razonable del backlog premium (12/14 — los 2 restantes, marketplace multi-vendor y multi-tienda, son cambios de modelo de negocio, no features autocontenidas). Mismo ciclo de validación por ítem que el resto del backlog (Docker + build + Playwright + suite e2e antes de dar cada uno por cerrado).

**Multi-warehouse / drop shipping** — `Warehouse` + `WarehouseStock` (desglose real de stock por almacén; `ProductVariant.stock` pasa a ser un total cacheado, mismo patrón que `User.loyalty_points`). El checkout asigna almacén por prioridad ascendente con `updateMany`+guardia atómica (`allocateAndDecrementStock`, `src/lib/warehouse.ts`); un almacén de **drop shipping** tiene capacidad virtual — nunca bloquea una venta ni descuenta contador local. Las 4 rutas de restitución (webhook Stripe cancelado/reembolsado, cron `cleanup-pending-orders`, RMA `markReturnReceived`) comparten un único helper `restockToWarehouse`. Nuevo `/admin/warehouses` + matriz "Stock por almacén" en la ficha de producto.

- **Bug real encontrado en la validación:** el checkout tenía un guard temprano (`if (enforceStock && variant.stock < quantity) return 400`) que rechazaba pedidos que un almacén de drop shipping activo sí podía cubrir, porque el total cacheado de `ProductVariant.stock` excluye a propósito la capacidad virtual del drop shipping. Eliminado — la única fuente de verdad de disponibilidad es la asignación dentro de la transacción (§5.1).

**Veri*Factu (AEAT) — base local, no certificada** — `InvoiceRecord` + `InvoiceChainState`: cada factura genera un registro de facturación encadenado (huella SHA-256 enlazada a la anterior, QR normativo, desglose base/cuota de IVA) dentro de la misma transacción que asigna `invoice_number`. El encadenamiento se serializa bajo concurrencia con `SELECT ... FOR UPDATE` sobre un puntero singleton — a diferencia de `InvoiceCounter` (UPSERT atómico simple), aquí el valor nuevo depende del contenido del anterior. Nuevo `/admin/invoicing` con verificación de integridad de la cadena (recalcula huellas, detecta alteraciones retroactivas) y export CSV. **No es una integración certificada:** sin certificado digital real no hay firma electrónica ni envío a la AEAT en tiempo real — avisos explícitos en código y UI. Detalle completo en §5.9.

- **Bug real pre-existente encontrado en la validación (sin relación con Veri*Factu en origen):** la generación de PDF de facturas llevaba **rota desde que existe** (bloque Operativa, 2026-05-02) — nadie había probado el endpoint `/api/admin/orders/[id]/invoice.pdf` con una petición HTTP real hasta esta validación. `pdfkit` resuelve sus fuentes estándar (`.afm`) en runtime con `path.join(__dirname, 'data', ...)` sobre su propio código — un patrón que asume que el paquete vive intacto en `node_modules`. Next.js (tanto Turbopack en dev como el build de producción) empaquetaba `pdfkit` y reescribía ese `__dirname` a una ruta virtual del bundle (`/ROOT/...`) inexistente en disco ⇒ `new PDFDocument()` reventaba con `ENOENT` en cuanto pedía la primera fuente, en dev **y** en `next start`/producción/Docker. **Fix:** `serverExternalPackages: ['pdfkit']` en `next.config.ts` — saca el paquete del bundle y lo deja como `require()` real de Node en runtime. Patrón general: cualquier paquete que resuelva assets propios vía `__dirname`/rutas relativas a su propio código en runtime (no sólo imports estáticos) necesita ir en `serverExternalPackages`, o el bundler lo rompe en silencio hasta la primera petición real.
- De paso, corregidos: un bug de precedencia de operadores (`||` vs `? :`) en la resolución del nombre del cliente del PDF que ignoraba `users.name`/`guest_name` siempre que hubiera dirección de facturación, y `Order.tax` (antes hardcodeado a `0`) ahora refleja el desglose de IVA real (ver §5.1).
